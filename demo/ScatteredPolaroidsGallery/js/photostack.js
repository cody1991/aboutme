...
(function(window) {
    ...
    Photostack.prototype._initEvents = function() {
        ...
        document.getElementById('next').addEventListener('click', function() {
            var callback = function() {
                var next;
                var next = (self.current + 1) === self.itemsCount ? 0 : (self.current + 1);
                self._showPhoto(next);
            }
            if (self.flipped) {
                self._rotateItem(callback);
            } else {
        ...
        document.getElementById('prev').addEventListener('click', function() {
            var callback = function() {
                var prev;
                var prev = (self.current ) === 0 ? (self.itemsCount - 1) : (self.current - 1);
                self._showPhoto(prev);
            }
            if (self.flipped) {
                self._rotateItem(callback);
            } else {
